...
class GroupViewController: UIViewController,UITableViewDataSource,UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete  && theReceipts[indexPath.row].payedBy == theUserName{
            ...
            let alert = UIAlertController(title: "Error", message: "Receipt can only be deleved by \(theReceipts[indexPath.row].payedBy!)", preferredStyle: .alert)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let myPayedData = ref.child("groupData/\(self.theGroupID)/receipts/\(self.arrayofReceiptIds[indexPath.row])/payed/\(theUserName)").observeSingleEvent(of: .value){
                (snapshot) in
            let haveIPayed = snapshot.value as? String
            if(haveIPayed == nil || haveIPayed! != "false"){
                cell.backgroundColor = UIColor(red: 0.76, green: 0.88, blue: 0.87, alpha: 1.00)
            }
            else{
                cell.backgroundColor = UIColor(red: 0.96, green: 0.87, blue: 0.74, alpha: 1.00)
    ...
    func createTextLabel(index:Int)->String{
        ...
        var totalString = String(format: "%.2f", total ?? 0)
